...
class AddAnswerUseCaseTest {
    ...
    void getQuestionSuccessTest() {
        ...
        StepVerifier.create(getUseCase.apply(question.getId())).expectNextMatches(
                questionDTO1 -> {
                    assert questionDTO1.getAnswers().get(0).getQuestionId().equals("1");
                    assert questionDTO1.getAnswers().get(0).getAnswer().equals("bla bla bla");
                    assert questionDTO1.getAnswers().get(0).getUserId().equals("12");
                    assert questionDTO1.getAnswers().get(0).getPosition().equals(1);
                    return true;
                }
        ).verifyComplete();
